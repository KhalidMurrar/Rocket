// PID
volatile float Kp = 3.67, Kd = 0.185;
float derivative = 0.0;
float error, lastError, output;
unsigned long prevMillis;

volatile int baseSpeed = 110;
...
void loop() {
  ...
  //if (leftDist == maxDist || rightDist == maxDist ) {
    //error = frontLeftDist - frontRightDist;
  //} else {
...
void readUltrasonic(UltrasonicSensor &s) {
  ...
  long duration = pulseIn(s.echoPin, HIGH, 3500);
