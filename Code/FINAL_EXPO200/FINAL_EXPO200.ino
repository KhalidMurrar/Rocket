...
void setup() {
  ...
  xTaskCreatePinnedToCore(BT_TuningTask, "BT_Task", 4000, NULL, 1, NULL, 0);
}

void BT_TuningTask(void *pvParameters) {
  for (;;) {
    if (SerialBT.available()) {
      char type = SerialBT.read();        // Expects 'P', 'D', or 'S'
      float val = SerialBT.parseFloat();  // The new value

      if (type == 'P') {
        Kp = val;
        SerialBT.print("New Kp: ");
        SerialBT.println(Kp);
      } else if (type == 'D') {
        Kd = val;
        SerialBT.print("New Kd: ");
        SerialBT.println(Kd);
      } else if (type == 'S') {
        baseSpeed = (int)val;
        SerialBT.print("New BaseSpeed: ");
        SerialBT.println(baseSpeed);
      }
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);  // Don't hog Core 0
  }
}
